...
namespace BCEdit180.Core.Editor.Classes {
    public class SourceCodeViewModel : BaseViewModel {
        ...
        public bool IsGenerating {
            ...
            set => this.RaisePropertyChanged(ref this.isGenerating, value);
        }
        ...
        public SourceCodeViewModel(ClassViewModel vm) {
            this.Class = vm;
            this.GenerateCommand = new AsyncRelayCommand(this.GenerateCode);
        }

        public async Task GenerateCode() {
            this.IsGenerating = true;
            string code = await Task.Run(this.GenerateCodeAsync);
        ...
        private string GenerateCodeAsync() {
            ...
            sb.Append(GetTypeName(this.Class.ClassInfo.FullClassName));
            sb.Append(" extends ").Append(GetTypeName(this.Class.ClassInfo.SuperName));
            if (this.Class.ClassInfo.Interfaces.Count > 0) {
                sb.Append(" implements ").Append(string.Join(", ", this.Class.ClassInfo.Interfaces.Select(a => GetTypeName(a.FullName))));
            }

            sb.Append(" {\n");

            sb.Append($"    // ({this.Class.FieldList.Fields.Count}) Fields \n");
            foreach (FieldViewModel field in this.Class.FieldList.Fields) {
                sb.Append("    ");
            ...
            sb.Append($"\n    // ({this.Class.MethodList.Methods.Count}) Methods \n");
            foreach (MethodViewModel method in this.Class.MethodList.Methods) {
