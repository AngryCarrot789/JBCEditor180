...
namespace JavaAsm {
    ...
    public class ClassNode {
        ...
        internal void Parse(ClassReaderState readerState) {
            this.SourceFile = (this.GetAttribute(PredefinedAttributeNames.SourceFile)?.ParsedAttribute as SourceFileAttribute)?.Value;
            this.SourceDebugExtension = (this.GetAttribute(PredefinedAttributeNames.SourceDebugExtension)?.ParsedAttribute as SourceFileAttribute)?.Value;
            this.Signature = (this.GetAttribute(PredefinedAttributeNames.Signature)?.ParsedAttribute as SignatureAttribute)?.Value;
            {
                AttributeNode attribute = this.GetAttribute(PredefinedAttributeNames.RuntimeInvisibleAnnotations);
                if (attribute != null)
                    this.InvisibleAnnotations = (attribute.ParsedAttribute as RuntimeInvisibleAnnotationsAttribute)?.Annotations;
            }
            {
                AttributeNode attribute = this.GetAttribute(PredefinedAttributeNames.RuntimeVisibleAnnotations);
                if (attribute != null)
                    this.VisibleAnnotations = (attribute.ParsedAttribute as RuntimeVisibleAnnotationsAttribute)?.Annotations;
            }
            this.IsDeprecated = this.GetAttribute(PredefinedAttributeNames.Deprecated)?.ParsedAttribute != null;
            this.EnclosingMethod = this.GetAttribute(PredefinedAttributeNames.EnclosingMethod)?.ParsedAttribute as EnclosingMethodAttribute;
            {
                AttributeNode attribute = this.GetAttribute(PredefinedAttributeNames.InnerClasses);
                if (attribute != null)
