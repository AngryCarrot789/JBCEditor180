        ...
        internal static ElementValue Parse(Stream stream, ClassReaderState readerState) {
            ...
            switch (elementValue.Tag) {
                ...
                case ElementValueTag.String:
                    elementValue.ConstValue = readerState.ConstantPool.GetEntry<Utf8Entry>(Binary.BigEndian.ReadUInt16(stream)).Value;
                    break;
                case ElementValueTag.Enum:
                    elementValue.EnumConstValue = new EnumConstValueType {
                        TypeName = TypeDescriptor.Parse(readerState.ConstantPool.GetEntry<Utf8Entry>(Binary.BigEndian.ReadUInt16(stream)).Value),
                        ConstName = readerState.ConstantPool.GetEntry<Utf8Entry>(Binary.BigEndian.ReadUInt16(stream)).Value
                    };
                    break;
                case ElementValueTag.Class:
                    elementValue.Class = TypeDescriptor.Parse(readerState.ConstantPool.GetEntry<Utf8Entry>(Binary.BigEndian.ReadUInt16(stream)).Value, true);
                    break;
