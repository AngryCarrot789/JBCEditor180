                ...
                switch (opcode) {
                    ...
                    case Opcode.INVOKEDYNAMIC:
                        ...
                        instructions.Add(currentPosition, new InvokeDynamicInstruction {
                            Name = callSiteSpecifier.NameAndType.Name.Value,
                            Descriptor = MethodDescriptor.Parse(callSiteSpecifier.NameAndType.Descriptor.Value),
                    ...
                    case Opcode.MULTIANEWARRAY:
                        instructions.Add(currentPosition, new MultiANewArrayInstruction {
                            Type = new ClassName(readerState.ConstantPool.GetEntry<ClassEntry>(Binary.BigEndian.ReadUInt16(codeStream)).Name.Value),
                    ...
                    case Opcode.INVOKEVIRTUAL: {
                        ...
                        if (opcode == Opcode.INVOKEINTERFACE) {
                            ...
                            int requiredSizeOfArguments =
                                MethodDescriptor.Parse(methodReferenceEntry.NameAndType.Descriptor.Value).ArgumentTypes.Sum(x => x.SizeOnStack) + 1;
                        ...
                        instructions.Add(currentPosition, new MethodInstruction(opcode) {
                            Owner = new ClassName(methodReferenceEntry.Class.Name.Value),
                            Descriptor = MethodDescriptor.Parse(methodReferenceEntry.NameAndType.Descriptor.Value),
                            Name = methodReferenceEntry.NameAndType.Name.Value
                        });
                    }
                        break;
                    ...
                    case Opcode.PUTSTATIC:
                        ...
                        instructions.Add(currentPosition, new FieldInstruction(opcode) {
                            Owner = new ClassName(fieldReferenceEntry.Class.Name.Value),
                            Descriptor = TypeDescriptor.Parse(fieldReferenceEntry.NameAndType.Descriptor.Value),
                            Name = fieldReferenceEntry.NameAndType.Name.Value
                        });
                        break;
                    ...
                    case Opcode.LDC: {
                        ...
                        switch (constantPoolEntry) {
                            ...
                            case FloatEntry fe:
                                ...
                                break;
                            case StringEntry se:
                                ldcVal = se.Value.Value;
                                break;
                            case ClassEntry ce:
                                ldcVal = new ClassName(ce.Name.Value);
                                break;
                            case MethodTypeEntry mte:
                                ldcVal = MethodDescriptor.Parse(mte.Descriptor.Value);
                                break;
                    ...
                    case Opcode.LDC2_W: {
                        ...
                        switch (constantPoolEntry) {
                            ...
                            case StringEntry se when opcode == Opcode.LDC_W:
                                ldcValue = se.Value.Value;
                                break;
                            case ClassEntry ce when opcode == Opcode.LDC_W:
                                ldcValue = new ClassName(ce.Name.Value);
                                break;
                            case MethodTypeEntry mte when opcode == Opcode.LDC_W:
                                ldcValue = MethodDescriptor.Parse(mte.Descriptor.Value);
                                break;
                            case MethodHandleEntry mhe when opcode == Opcode.LDC_W:
                                ldcValue = new Handle {Descriptor = mhe.ReferenceKind.IsFieldReference() ? TypeDescriptor.Parse(mhe.Reference.NameAndType.Descriptor.Value) : (IDescriptor) MethodDescriptor.Parse(mhe.Reference.NameAndType.Descriptor.Value), Name = mhe.Reference.NameAndType.Name.Value, Owner = new ClassName(mhe.Reference.Class.Name.Value)};
                                break;
